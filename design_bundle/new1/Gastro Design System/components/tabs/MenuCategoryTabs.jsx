      ...
      {categories.map((c) => {
        ...
        return (
          <button key={key} type="button" role="tab" aria-selected={active} onClick={() => onChange(key)} style={{
            ...
            padding: 'var(--space-md) var(--space-xxs)', WebkitTapHighlightColor: 'transparent',
            fontSize: 'var(--font-size-body)', lineHeight: 'var(--line-height-body)', fontWeight: 'var(--font-weight-semibold)',
            color: active ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)',
            borderBottom: active ? 'var(--border-width-default) solid var(--color-brand-primary)' : 'var(--border-width-default) solid transparent',
