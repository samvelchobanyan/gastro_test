...
**Typography rule (mandatory).** Never render raw text with ad-hoc font sizing. Every piece of text — in new components or edits to existing ones — MUST use a style from the Type Scale (the `--font-size-*` / `--line-height-*` / `--font-weight-*` tokens, mirrored in the "Type scale" foundation card). Set `font-size`, `line-height` and `font-weight` from those tokens together as a matched pair; never hand-pick a pixel size outside the scale. This keeps the 4px baseline rhythm intact and ensures fonts inherit correctly.

**Font-weight rule (mandatory).** Every text element MUST match its Type Scale style on all three axes — size, line-height AND weight. Each type style has a defined weight (Display/H1/H2/H3 bold · body/caption regular · overline semibold; see the `--text-*` shorthands in `tokens/typography.css`). When a component uses a Type Scale style, apply that style's weight too — e.g. a Section Header using Heading 3 renders bold. Do NOT invent weights outside the scale or override a style's weight ad-hoc; if a design needs a different weight, change the Type Scale, not the component.
...
**components/** — reusable primitives (`.jsx` + `.d.ts` + `.prompt.md` + `@dsCard`), organized by concern. 68 components total:

- `buttons/` — Button
- `icon-button/` — IconButton
- `fab/` — Fab
- `inputs/` — TextInput, TextArea, Dropdown, Accordion
- `search/` — SearchBar, SearchFiltersPanel
- `forms/` — Checkbox, Radio, RadioGroup, Switch, Slider, Stepper, FormGroup
- `navigation/` — BottomNavigation
- `tabs/` — TabBar, MenuCategoryTabs
...
- `chip/` — Chip
- `rating/` — RatingStars
- `list-items/` — AdvancedListItem, SimpleListItem
- `feedback/` — BottomSheet, Dialog, Toast, ProgressBar, Spinner, Skeleton, EmptyState, ErrorState
- `tooltip/` — Tooltip
- `media/` — MapView (used by LiveOrderTrackingMap; no card of its own)
- `pickers/` — Calendar, DatePicker, TimePicker
- `news/` — NewsListing (vertical, horizontal)
- `section-header/` — SectionHeader
- `menu/` — MenuItemCard, ItemCustomizationSheet, CartLineItem
- `cart/` — CartSummaryBar, OrderTotalBreakdown, PromoCodeInput
- `restaurant/` — RestaurantCard, RestaurantHeader, BranchCard, DeliveryPickupToggle, AddressSelector, DeliveryTimeEstimate
- `filter/` — Filter, FilterBar
- `quick-action-tiles/` — QuickActionTile, QuickActionTiles
- `order/` — OrderStatusTimeline, LiveOrderTrackingMap, OrderHistoryCard, ReorderButton, RatingReviewPrompt
- `checkout/` — TipSelector, PaymentMethodCard, PaymentMethodPicker
- `loyalty/` — LoyaltyRewardsBadge, LoyaltyPointsCard, MembershipTierPill, Wallet
- `voucher/` — Voucher
- `product-cards/` — BuyAgainCard, FavouriteVerticalCard, FavouriteVerticalCard
- `confirmations/` — ReservationConfirmationCard
- `greeting/` — Greeting
- `stories/` — Stories

This build-once, neutral inventory covers the full client component brief (`…Component List.txt`) — general UI primitives plus the restaurant/ordering domain set (menu, cart, checkout, tracking, reservations, loyalty). UI-kit screens (full product recreations) are the natural next step and are not yet built.
